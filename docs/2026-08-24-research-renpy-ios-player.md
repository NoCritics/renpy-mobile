...
Current stack (8.5.3, released 2026-05-15): **CPython 3.12.7** (confirmed for the desktop
SDK build only — the `renios` build's bundled interpreter version has not been separately
verified and must not be assumed identical; see the design spec §4), **SDL3** + SDL3_image,
MetalANGLE (GLES→Metal), FFmpeg 4.3.1, freetype 2.13.3, harfbuzz 8.0.1, fribidi,
libavif/aom/yuv/jpeg/png/webp, openssl 3.3.2, assimp, **pyobjus** (Python→Objective-C
bridge, the iOS analogue of pyjnius).
